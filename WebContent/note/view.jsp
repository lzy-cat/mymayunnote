			...
			<div class="container-fluid">
				<div class="row" style="padding-top: 20px;">
					<c:if test="${empty noteList }">
						<h2>请先添加云记类型！</h2>
						<h3>
							<a href="type?actionName=list">添加类型</a>
						</h3>
					</c:if>
					<c:if test="${!empty noteList }">
						<div class="col-md-12">
							<form id="noteForm" class="form-horizontal" method="post"
								action="note">
										...
										<c:choose>
											<c:when test="${!empty resultInfo.result.typeId }">
												<select id="typeId" class="form-control" name="typeId">
													...
													<c:forEach items="${noteList }" var="item">
														<option
															<c:if test="${resultInfo.result.typeId == item.typeId }">selected="selected"</c:if>
											...
											<c:otherwise>
												<select id="typeId" class="form-control" name="typeId">
													...
													<c:forEach items="${noteList }" var="item">
														<option
															<c:if test="${noteInfo.typeId == item.typeId }">selected="selected"</c:if>
															value="${item.typeId }">${item.typeName }</option>
													</c:forEach>
												</select>
											</c:otherwise>
										...
										<c:choose>
											...
											<c:when test="${!empty resultInfo.result.title }">
												<input class="form-control" name="title" id="title"
													placeholder="云记标题" value="${resultInfo.result.title }">
											</c:when>
											<%-- 当进入修改云记页面时的填充数据 --%>
											<c:otherwise>
												<input class="form-control" name="title" id="title"
													placeholder="云记标题" value="${noteInfo.title }">
											</c:otherwise>
										</c:choose>
									</div>
								</div>
								...
								<div class="form-group">
									<div class="col-sm-offset-5 col-sm-4">
										<input type="button" class="btn btn-primary"
											onclick="saveNote();" value="保存"> <font id="error"
											color="red">${resultInfo.msg }</font>
									</div>
								</div>
							</form>
						</div>
